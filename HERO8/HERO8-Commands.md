### GoPro HERO8 Commands, Status and Notes

> **Nota sulla provenienza**: la maggior parte dei comandi qui sotto e' identica
> a quella di [HERO7](/HERO7/HERO7-Commands.md) — questo e' il comportamento
> noto e documentato dal README principale del repo ("For GoPro Hero8... For
> other commands, use Hero7 commands"). Per la Webcam mode (GoPro Connect via
> USB), vedi invece [GoPro-Connect.md](/HERO8/GoPro-Connect.md), basata su
> HERO9 + verifiche dirette su hardware reale.
>
> In fondo a questo file c'e' una sezione **"Additional/Undocumented Commands"**
> con endpoint aggiuntivi scoperti nel 2026-08 estraendo le stringhe dal
> firmware ufficiale HERO8 Black v2.51.00 (`DATA.bin` dentro `UPDATE.zip`),
> molti dei quali verificati live contro una camera reale. Vedi anche
> [Firmware-Endpoints.md](/HERO8/Firmware-Endpoints.md) e
> [Testing-Notes.md](/HERO8/Testing-Notes.md).

Sample /status (via USB, IP tipo `172.XX.XXX.51`)
...
## Additional/Undocumented Commands (scoperti 2026-08, firmware v2.51.00)

Estratti eseguendo `strings` sul binario `DATA.bin` dentro il firmware
ufficiale HERO8 Black (`UPDATE.zip`, versione 02.51.00,
https://github.com/KonradIT/gopro-firmware-archive). Tutti i path vanno sotto
`/gp/gpControl/` a meno di indicazione diversa. Testati live uno alla volta
(mai in rapida sequenza — vedi [Testing-Notes.md](/HERO8/Testing-Notes.md)
sul perche').

Legenda esito: ✅ risponde 200 con dati utili · ⚠️ risponde 500 perche' manca
un parametro obbligatorio (endpoint reale, sintassi non ancora determinata) ·
❌ 404, il path non esiste con questa esatta forma.

### Bluetooth (oltre a quelli gia' in HERO7-Commands.md sopra)

| Comando | Esito | Note |
|---|---|---|
| `command/ble/pairing_available` | ✅ | |
| `command/ble/pairing_phase` | ⚠️ | serve stato di pairing attivo |
| `command/ble/pair/status` | ✅ `{"pairing_status":0}` | |
| `command/ble/scan/list` | ✅ | lista risultati scan |
| `command/ble/configure` | ✅ `{"auto_connect":1}` | |
| `command/ble/control` | ⚠️ | serve parametro |
| `command/ble/pair/cancel` | ⚠️ | serve stato di pairing attivo |
| `command/ble/whitelist/configure` | non testato | |
| `command/ble/whitelist/device` | non testato | |
| `command/ble/whitelist/list` | ✅ | lista vuota se nessun device |

### Stato/Sistema

| Comando | Esito | Note |
|---|---|---|
| `command/mobile_offload/get_state` | ✅ | **Molto utile**: `{"mobile_offload_state":0,"new_media":1,"battery_ok":1,"sd_card_ok":0,"camera_busy":0,"paused":0,...}` — stato sintetico in un colpo solo |
| `command/mobile_offload/clear_new_media_flag` | non testato | |
| `command/system/get_beacon` | ✅ `{"uuid":"...","major":0,"minor":0}` | |
| `command/system/enable_beacon?p=0/1` | ✅ | |
| `command/system/set_beacon` | non testato | |
| `command/system/logs_list` | ⚠️ | |
| `command/system/logs_clear` | ⚠️ | serve parametro |
| `command/system/logs_prepare` | non testato | |
| `command/system/notify_event` | non testato | |
| `command/system/suspend_resume_state` | ⚠️ | serve parametro |
| `command/set_client_info` | non testato | |
| `command/set_mode?p=N` | ⚠️ | alias di `command/mode`? non ha funzionato con p=0 |
| `command/set_preset` | non testato | serve un preset id valido |
| `command/setup/date_time` | non testato dal vivo (solo verificato che esiste), vedi versione HERO3 sopra per il formato esadecimale |
| `command/button_set` | non testato | |
| `command/onscreen_shortcuts_reset` | ✅ | |
| `command/photo_timer` | ⚠️ | serve parametro |
| `command/sub_mode?mode=X&sub_mode=Y` | ✅ | stessa sintassi HERO7 |

### ⚠️ PERICOLOSI — non testati di proposito

| Comando | Perche' evitarlo |
|---|---|
| `command/system/factory/reset`, `command/system/factory_reset` | azzera tutte le impostazioni della camera |
| `command/fwupdate/download/start`, `/cancel`, `/done` | avvia una vera procedura di aggiornamento firmware; interromperla puo' danneggiare la camera |
| `command/system/shutdown` | spegne la camera (chiude la sessione USB attiva) |
| `command/wireless/ap/control` | puo' disattivare il WiFi della camera stessa |

### Protune reset (per modalita')

| Comando | Esito |
|---|---|
| `command/protune/reset` | ✅ |
| `command/video/protune/reset` | ✅ |
| `command/photo/protune/reset` | ⚠️ (serve essere in modalita' Photo) |
| `command/multi_shot/protune/reset` | ⚠️ (serve essere in modalita' MultiShot) |

### Storage (oltre a delete/last, delete/all, tag_moment gia' in HERO7-Commands.md)

| Comando | Esito |
|---|---|
| `command/storage/delete/group` | ⚠️ serve parametro |
| `command/storage/tag_moment/playback` | ⚠️ serve parametro (path file) |
| `command/storage/tag_moment/playback/photo` | ⚠️ serve parametro |

### Wireless (oltre a scan/list/save/select gia' in HERO7-Commands.md)

| Comando | Esito |
|---|---|
| `command/wireless/pair/cancel` | ✅ |
| `command/wireless/rc/pair` | non testato |
| `command/wireless/rc/pair/cancel` | ✅ |
| `command/wireless/band/select` | ⚠️ serve parametro |
| `command/wireless/ssid/delete` | ⚠️ serve parametro |
| `command/wireless/ap/ssid` | ⚠️ serve parametro (per impostare il nome della rete della camera) |
| `command/wirelessOffload/pause`, `/start`, `/stop` | ❌ 404 — il path esatto e' probabilmente diverso (forse sotto `mobile_offload`?) |

### Transcode (oltre a request/status/cancel gia' documentati sopra)

| Comando | Esito |
|---|---|
| `command/transcode/history` | ✅ `{"history":[]}` |
| `command/transcode/photo_to_video` | non testato |
| `command/transcode/video_to_video` | non testato |

### Live view

| Comando | Esito |
|---|---|
| `command/liveview/exposure_mode` | ⚠️ serve parametro |

### Prefissi di primo livello non esplorati

Trovati nel firmware ma non ancora indagati in dettaglio (probabile utilizzo
solo interno o per l'app mobile):

`/gp/gpBacpac` (modulo batteria esterna) · `/gp/gpCamera` · `/gp/gpCert`
(verifica certificati) · `/gp/gpEMS` · `/gp/gpMediaList` / `/gp/gpMediaListEx`
(lista file SD, presente anche su HERO9/HERO7 come Media Browsing) ·
`/gp/gpMediaMetadata` / `/gp/gpMediaMetadata2` · `/gp/gpMediaOp` ·
`/gp/gpSoftUpdate` / `/gp/gpUpdate` (aggiornamento software) · `/gp/gpTurbo`.
